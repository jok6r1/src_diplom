...
pool.on('error', (err : Error) => console.error('❌ Ошибка подключения к базе данных:', err.message));
...
// Registration
app.post('/pgadmin/register', async (req : any, res : any) => {
...
// Login
app.post('/pgadmin/login', async (req : any, res : any) => {
...
// Get users
app.get('/pgadmin/getUsers', async (req : any, res : any) => {
  const { active, role, limit = 100, offset = 0 } = req.query;
  try {
    if (active && !['true', 'false'].includes(active.toString())) return res.status(400).json({ error: 'Invalid active parameter' });
...
app.get('/pgadmin/check-user', async (req : any, res : any) => {
...
// Get hidden IPs
app.get('/pgadmin/hide_ip', async (req : any, res : any) => {
  try {
...
// Store anomalies
app.post('/pgadmin/anomalies', async (req : any, res : any) => {
  const data = req.body;
...
app.get('/pgadmin/traffic/user/:userId', async (req : any, res : any) => {
...
// Get traffic by IP
app.get('/pgadmin/traffic/ip/:ip', async (req : any, res : any) => {
...
app.get('/pgadmin/anomalies/last15min', async (req : any, res : any) => {
...
// Get traffic by ID
app.get('/pgadmin/traffic/:id', async (req : any, res : any) => {
...
app.get('/pgadmin/users-and-traffic', async (req : any, res : any) => {
  try {
    ...
    const formattedTraffic = trafficResult.rows.map((row : any) => ({
      id: row.id,
      user_id: Number(row.user_id),
      ip: row.ip,
      timestamp: row.timestamp.toISOString(),
...
// Get latest traffic
app.get('/pgadmin/latest-traffic', async (req : any, res : any) => {
  try {
    ...
    if (result.rows.length === 0) return res.status(404).json({ success: false, error: 'No data found' });
...
// Execute SQL
app.post('/pgadmin/execute-sql', async (req : any, res : any) => {
...
app.get('/pgadmin/execute-sql/check-connection', async (req : any, res : any) => {
  try {
...
// List files
app.get('/pgadmin/list-files', (req : any, res : any) => {
  fs.readdir(FILES_DIR, (err : Error, files : any) => {
    if (err) {
...
// Download file
app.get('/pgadmin/download/:filename', (req : any, res : any) => {
  const filename = req.params.filename;
  const filePath = path.join(FILES_DIR, filename);
  if (!/^[\w-]+\.(exe|deb)$/.test(filename) || !fs.existsSync(filePath)) {
    return res.status(400).json({ error: 'Invalid or missing file' });
  }
  res.download(filePath, filename, (err : Error) => err && console.error('Download error:', err));
});
